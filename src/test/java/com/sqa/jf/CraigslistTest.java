...
public class CraigslistTest extends BasicTest {
	...
	public void craigslistTest(String keywords, int minExpectedResults, int maxExpectedResults)
		...
		// Convert the WebElement into a selectCat object Select
		Select selectCat = new Select(catArea);
		// Selec tthe option by visible text
		selectCat.selectByVisibleText("jobs");
		// Capturing the text within the element which shows number of results
		...
		if (results.equalsIgnoreCase("no results")) { // "no results"
			// Set to 0 because there are no results
			totalResults = 0;
			// Check that there is a length of more than only one (which is no
			// "of" present scenario)
		...
		} else { // eg:"1 to 68" elements: "1 " and " 68"
			totalResults = Integer.parseInt(results.split("to")[1].trim());
			// System.out.println("Total Results:" + totalResults);
		}
		// Capture a screenshot:
		// File set to the WebDriver instance, casted to a TakeScreenshot to
		// make getScreenshotAs available
